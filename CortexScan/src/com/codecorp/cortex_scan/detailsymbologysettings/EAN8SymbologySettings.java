...
public class EAN8SymbologySettings extends PreferenceActivity implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        m2DigitSupplemental.setChecked(ean8Properties.isSupplemental2DigitDecodingEnabled(getApplicationContext()));

        m5DigitSupplemental = (CheckBoxPreference) getPreferenceScreen().findPreference("ean8_5digit_supp");
        m5DigitSupplemental.setChecked(ean8Properties.isSupplemental5DigitDecodingEnabled(getApplicationContext()));

        mAddSpace = (CheckBoxPreference) getPreferenceScreen().findPreference("ean8_add_space");
        mAddSpace.setChecked(ean8Properties.isAddSpaceEnabled(getApplicationContext()));

        mRequireSupplemental = (CheckBoxPreference) getPreferenceScreen().findPreference("ean8_require_supp");
        mRequireSupplemental.setChecked(ean8Properties.isRequireSupplemental(getApplicationContext()));

        loadSettings();

        if (myApplication.mCortexDecoderLibrary.isLicenseActivated()) {
            ean8Properties.setEnabled(getApplicationContext(), true);
        }
